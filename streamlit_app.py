...
import hashlib
import time   # ← add this line if it’s not already there

# Add this right after the imports (before any functions)
...
# ────── REPLACE YOUR WHOLE OLD get_df() WITH THIS ONE ──────
@st.cache_data(ttl=3600)
def get_df(_cache_buster, clinic_filter=None):   # ← note the new _cache_buster parameter
    conn = sqlite3.connect(DB_PATH)
...
with tab3:
    ...
    if df.empty:
        ...
        if st.button("Issue Medicine", type="primary"):
            ...
            st.success(f"Issued {issue_qty} × {selected_row.drug_name} (stock now: {new_qty})")
            st.rerun()  # ← THIS REFRESHES EVERYTHING INSTANTLY
